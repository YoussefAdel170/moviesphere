// src/hooks/Filter/useTvFilters.ts
...
export function useTvFilters() {
  ...
  const { filters } = useAppSelector((state) => state.tv);
  ...
  const handleApply = () => {
    dispatch(setTvGenresFilter(localGenres));
    dispatch(setTvYearRangeFilter([localYearMin, localYearMax]));
    dispatch(setTvVoteAverageFilter(localVoteAvg));
    dispatch(setTvSortByFilter(localSortBy));
    setAnchorEl(null);
  };
